...
async function run() {
    ...
    for (const arg of componentArgs) {
        ...
        // If variant exists in meta.variants, use it
        if (meta.variants && meta.variants[variantName]) {
            files = meta.variants[variantName].files;
        }
        // Else if no variant specified, use 'default' variant
        else if (variantName === "default" && meta.variants && meta.variants["default"]) {
            files = meta.variants["default"].files;
        }
        // Else error
        else {
